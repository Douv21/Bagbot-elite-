...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildRolesSelectHTML(selectedRoleId = '') {
    const selectedIds = String(selectedRoleId || '').split(',').map(s => s.trim());
  ...
  function openEmojiPickerPopover(targetInput, targetBtn) {
    let existing = document.getElementById('emoji-picker-popover');
    if (existing) existing.remove();
    ...
    popover.querySelector('#close-emoji-popover').addEventListener('click', () => popover.remove());

    const closeOnClickOutside = (e) => {
      if (!popover.contains(e.target) && !targetBtn.contains(e.target) && !targetInput.contains(e.target)) {
        popover.remove();
        document.removeEventListener('click', closeOnClickOutside);
      }
    };
  ...
  // Lier le sélecteur de rôle du mode simple au popup de recherche
  const newButtonRoleEl = document.getElementById('new-button-role');
  const newButtonRoleTagsEl = document.getElementById('new-button-role-tags');
  if (newButtonRoleEl) {
    newButtonRoleEl.addEventListener('change', () => {
      if (newButtonRoleTagsEl) updateRoleSelectTags(newButtonRoleEl, newButtonRoleTagsEl);
    });
    if (newButtonRoleTagsEl) {
      newButtonRoleTagsEl.style.cursor = 'pointer';
      newButtonRoleTagsEl.title = 'Cliquer pour rechercher et sélectionner des rôles';
      newButtonRoleTagsEl.addEventListener('click', (e) => {
        if (!e.target.classList.contains('btn-remove-role-tag')) {
          openRolePickerPopover(newButtonRoleEl, newButtonRoleTagsEl);
        }
      });
    }
  }
  ...
  document.getElementById('btn-add-autorole-button')?.addEventListener('click', () => {
    ...
    // Reset inputs & button icon
    if (roleSelectEl) {
      Array.from(roleSelectEl.options).forEach(o => o.selected = false);
      if (newButtonRoleTagsEl) updateRoleSelectTags(roleSelectEl, newButtonRoleTagsEl);
    }
    ...
    if (btnAdd) {
      ...
      btnAdd.title = 'Ajouter le bouton';
    }
      ...
      combined.forEach(item => {
        ...
        itemCard.querySelector('.btn-load-embed').addEventListener('click', () => {
          ...
          if (!item.is_recurring) {
            safeSetVal('simple_embed_edit_msg_id', item.message_id || item.id || '');
          ...
          safeSetVal('simple-embed-preview-footer-text', item.footer_text || item.footer || '');
          ...
          if (item.is_recurring) {
            ...
            .then(data => {
              if (data.success) {
                ...
                if (typeof showToast === 'function') showToast('Erreur: ' + (data.error || 'Impossible de supprimer'), true);
              }
            })
            .catch(err => showToast(err.message, true));
          } else {
