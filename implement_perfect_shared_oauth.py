...
# Update /login in dashboard.js to save redirect query and call req.session.save before redirecting
...
print("src/dashboard.js updated with persistent postLoginRedirect!")

# 2. Update src/dashboard2.js
with open('src/dashboard2.js', 'r', encoding='utf-8') as f:
    d2_code = f.read()

# Update session store to use user_sessions table and cookie bagbot-elite.sid
...
# Update /login in dashboard2.js to route through whitelisted OAuth2 endpoint and redirect back to 49602
...
print("src/dashboard2.js updated to use shared user_sessions store and redirect login via port 49601!")
