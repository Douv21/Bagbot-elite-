...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('boutique')
    .setDescription('Afficher les articles ou acheter dans la boutique')
    .addSubcommand(sub =>
      sub
        .setName('afficher')
        .setDescription('Afficher la liste des articles de la boutique')
    )
    .addSubcommand(sub =>
      sub
        .setName('acheter')
  ...
  async execute(interaction) {
    ...
    if (subcommand === 'afficher') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription('Voici les articles disponibles à l\'achat. Utilisez \`/boutique acheter <nom>\` pour en acquérir un !')
        .setColor('#5865F2')
    ...
    if (subcommand === 'acheter') {
      ...
      if (!item) {
        return interaction.reply({ content: `❌ L'article **${itemName}** n'existe pas dans la boutique. Utilisez \`/boutique afficher\` pour voir les articles disponibles.`, ephemeral: true });
      }
      ...
      if (isSuite) {
        ...
        try {
          ...
          // Note: voice_channel_id est passé comme null car nous n'ouvrons plus qu'un channel textuel spécial
