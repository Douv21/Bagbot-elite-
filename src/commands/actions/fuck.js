...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('fuck')
    .setDescription("Baiser quelqu\'un")
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'fuck');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('fuck', 'Baiser quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} se fait plaisir en solo...`
        : `${author} prend sauvagement ${target} dans une étreinte charnelle et torride ! || ${author} baise et pénètre passionnément ${target} pour une nuit de pure folie... || L\'union charnelle de ${author} et ${target} atteint des sommets de luxure !`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'fuck');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'fuck');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('fuck');
      } catch (e) {
