...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('tromper')
    .setDescription("Tromper quelqu\'un")
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'tromper');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('tromper', 'Tromper quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} se ment à lui-même.`
        : `${author} s\'échappe discrètement pour passer un moment secret et torride avec ${target}... || Dans l\'ombre, ${author} cède à la tentation et trompe ses proches avec ${target}... || ${author} partage une relation secrète, interdite et passionnée avec ${target}.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'tromper');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'tromper');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('tromper');
      } catch (e) {
