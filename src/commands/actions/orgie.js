...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const aiPhrase = await generateAiActionPhraseFast('orgie', 'Organiser une orgie', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    const gifUrl = getValidActionGifUrl(guildId, 'orgie');

    const randomGif = getValidActionGifUrl(guildId, 'orgie');
    if (randomGif) {
