...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'lit');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('lit', 'Aller au lit avec quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} s\'allonge sur le lit en rêvant de compagnie.`
        : `${author} entraîne ${target} sous les draps pour une nuit torride et agitée... || Le lit grince sous les mouvements passionnés de ${author} et ${target}... || ${author} et ${target} se rejoignent sur le lit pour un corps-à-corps brûlant.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'lit');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    const embed = new EmbedBuilder()
      .setTitle("🛏️ Partie de Jambes en l\'air")
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'lit');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('lit');
      } catch (e) {
