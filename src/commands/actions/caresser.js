...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'caresser');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('caresser', 'Caresser doucement quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
