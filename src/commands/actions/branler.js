...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('branler')
    .setDescription("Branler quelqu\'un")
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'branler');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('branler', 'Branler quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ...
        : `${author} caresse et branle chaudement le sexe de ${target} ! || La main de ${author} caresse le sexe de ${target} avec un rythme de plus en plus torride... || ${author} excite ${target} d\'une main experte et fiévreuse.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'branler');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'branler');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('branler');
      } catch (e) {
