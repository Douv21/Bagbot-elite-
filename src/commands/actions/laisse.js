...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const aiPhrase = await generateAiActionPhraseFast('laisse', 'Mettre une laisse à quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    const gifUrl = getValidActionGifUrl(guildId, 'laisse');

    const randomGif = getValidActionGifUrl(guildId, 'laisse');
    if (randomGif) {
