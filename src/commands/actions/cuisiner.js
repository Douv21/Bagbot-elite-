...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const aiPhrase = await generateAiActionPhraseFast('cuisiner', 'Cuisiner pour quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    const gifUrl = getValidActionGifUrl(guildId, 'cuisiner');

    const randomGif = getValidActionGifUrl(guildId, 'cuisiner');
    if (randomGif) {
