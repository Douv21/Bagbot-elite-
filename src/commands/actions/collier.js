...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'collier');
      if (actReward) {
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'collier');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('collier');
      } catch (e) {
