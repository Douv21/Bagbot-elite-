...
module.exports = {
  data: new SlashCommandBuilder().setContexts([0, 1, 2]).setIntegrationTypes([0, 1])
    .setName('collier')
    .setDescription("Mettre un collier à quelqu\'un")
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const aiPhrase = await generateAiActionPhraseFast('collier', 'Mettre un collier à quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} s\'attache un collier autour du cou.`
        : `${author} passe doucement un collier autour du cou de ${target} pour en prendre possession. || ${author} attache un collier en cuir à ${target}, marquant leur lien charnel... || ${author} revendique ${target} en lui passant délicatement un collier autour du cou.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'collier');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    const gifUrl = getValidActionGifUrl(guildId, 'collier');

    const randomGif = getValidActionGifUrl(guildId, 'collier');
    if (randomGif) {
