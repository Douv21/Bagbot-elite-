...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const aiPhrase = await generateAiActionPhraseFast('mordre', 'Mordre quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    const gifUrl = getValidActionGifUrl(guildId, 'mordre');

    const randomGif = getValidActionGifUrl(guildId, 'mordre');
    if (randomGif) {
