...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const aiPhrase = await generateAiActionPhraseFast('reveiller', 'Réveiller quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    const gifUrl = getValidActionGifUrl(guildId, 'reveiller');

    const randomGif = getValidActionGifUrl(guildId, 'reveiller');
    if (randomGif) {
