...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const aiPhrase = await generateAiActionPhraseFast('calin', 'Faire un gros câlin à quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    const gifUrl = getValidActionGifUrl(guildId, 'calin');

    const randomGif = getValidActionGifUrl(guildId, 'calin');
    if (randomGif) {
