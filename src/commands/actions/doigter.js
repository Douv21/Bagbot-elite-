...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('doigter')
    .setDescription("Doigter quelqu\'un")
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'doigter');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('doigter', 'Doigter quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} explore son propre corps avec passion.`
        : `${author} glisse ses doigts agiles et humides pour caresser intimement ${target} ! || Le rythme des doigts de ${author} fait monter le désir chez ${target}... || ${author} procure des frissons extrêmes à ${target} d\'un doigté expert.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'doigter');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'doigter');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('doigter');
      } catch (e) {
