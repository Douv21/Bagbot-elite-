...
module.exports = {
  data: new SlashCommandBuilder().setContexts([0, 1, 2]).setIntegrationTypes([0, 1])
    .setName('reconforter')
    .setDescription("Réconforter quelqu\'un")
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const aiPhrase = await generateAiActionPhraseFast('reconforter', 'Réconforter quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} se console tout seul.`
        : `${author} enlace et réconforte tendrement ${target} contre son torse. || ${author} murmure des mots doux pour réconforter ${target} dans une étreinte protectrice... || ${author} offre un refuge chaleureux et complice pour réconforter ${target}.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'reconforter');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    const gifUrl = getValidActionGifUrl(guildId, 'reconforter');

    const randomGif = getValidActionGifUrl(guildId, 'reconforter');
    if (randomGif) {
