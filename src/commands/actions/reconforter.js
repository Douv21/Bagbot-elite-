...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'reconforter');
      if (actReward) {
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'reconforter');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('reconforter');
      } catch (e) {
