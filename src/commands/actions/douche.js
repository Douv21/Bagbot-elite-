...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('douche')
    .setDescription("Prendre une douche avec quelqu\'un")
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'douche');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('douche', 'Prendre une douche avec quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} prend une douche froide pour calmer ses ardeurs.`
        : `${author} rejoint ${target} sous l\'eau chaude de la douche pour un moment très sensuel... || Sous les jets d\'eau, ${author} savonne sensuellement le corps de ${target}... || L\'eau ruisselle sur les corps enlacés de ${author} et ${target} sous la douche.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'douche');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'douche');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('douche');
      } catch (e) {
