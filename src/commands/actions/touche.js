...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const aiPhrase = await generateAiActionPhraseFast('touche', 'Toucher quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} se touche le visage.`
        : `${author} caresse et effleure intimement les zones érogènes de ${target}... || Les mains de ${author} parcourent et touchent sensuellement le corps de ${target}... || ${author} effleure ${target} d\'un frisson de désir en le/la touchant.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'touche');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    const gifUrl = getValidActionGifUrl(guildId, 'touche');

    const randomGif = getValidActionGifUrl(guildId, 'touche');
    if (randomGif) {
