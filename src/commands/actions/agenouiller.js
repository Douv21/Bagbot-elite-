...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'agenouiller');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('agenouiller', 'S\'agenouiller devant quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} s\'agenouille devant son reflet.`
        : `${author} s\'agenouille amoureusement et se soumet devant ${target}. || ${author} s\'agenouille lentement devant ${target}, le regard brûlant de soumission... || Dans un geste plein de désir, ${author} s\'agenouille aux pieds de ${target}.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'agenouiller');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'agenouiller');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('agenouiller');
      } catch (e) {
