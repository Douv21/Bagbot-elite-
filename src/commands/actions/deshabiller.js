...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('deshabiller')
    .setDescription("Déshabiller quelqu\'un")
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'deshabiller');
      if (actReward) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} retire ses vêtements et se retrouve nu(e) devant le miroir.`
        : `${author} retire lentement et sensuellement chaque vêtement de ${target}... || Sous le regard ardent de ${author}, ${target} se fait déshabiller pièce par pièce... || ${author} déshabille doucement ${target} pour révéler ses charmes secrets.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'deshabiller');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    if (guildId) {
      ...
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('deshabiller');
      } catch (e) {
