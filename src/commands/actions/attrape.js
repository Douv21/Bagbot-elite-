...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('attrape')
    .setDescription("Attraper quelqu\'un")
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'attrape');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('attrape', 'Attraper quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} court en rond et s\'attrape lui-même !`
        : `${author} attrape ${target} par la taille et le plaque contre son corps chaud ! || ${author} surgit et attrape fermement ${target} pour l\'attirer dans ses bras... || ${author} attrape sensuellement ${target} et lui murmure un secret coquin.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'attrape');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'attrape');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('attrape');
      } catch (e) {
