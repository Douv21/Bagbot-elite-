...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const rewardConfig = guildId ? getActionReward(guildId, 'spank') : { min_money: 5, max_money: 15, min_karma: 1, max_karma: 3 };
    ...
    try {
      ...
      console.warn('[Action SPANK AI]', e.message);
    }

    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} s'inflige une petite fessée coquine en solo...`
        : `${author} donne une fessée retentissante et sensuelle à ${target} !`;
    }

    const embed = new EmbedBuilder()
      .setTitle("🍑 Fessée Coquine & Sensuelle")
