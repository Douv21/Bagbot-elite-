...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('rose')
    .setDescription("Offrir une rose à quelqu\'un")
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'rose');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('rose', 'Offrir une rose à quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} s\'offre une rose à lui-même.`
        : `${author} offre une rose parfumée à ${target} pour éveiller ses désirs 🌹. || ${author} fait glisser les pétales d\'une rose sur le corps de ${target}... || ${author} séduit ${target} en lui tendant une rose rouge passion.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'rose');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'rose');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('rose');
      } catch (e) {
