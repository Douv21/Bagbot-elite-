...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('seduire')
    .setDescription("Séduire quelqu\'un")
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'seduire');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('seduire', 'Séduire quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} se trouve irrésistible devant la glace.`
        : `${author} murmure des mots doux et déploie tout son charme pour séduire ${target} ! || Le regard envoûtant de ${author} se pose sur ${target} pour le/la séduire complètement... || ${author} fait chavirer le cœur et le corps de ${target} par ses gestes charmeurs.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'seduire');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'seduire');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('seduire');
      } catch (e) {
