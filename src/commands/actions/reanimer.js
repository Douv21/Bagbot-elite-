...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('reanimer')
    .setDescription("Réanimer quelqu\'un")
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'reanimer');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('reanimer', 'Réanimer quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} se fait du bouche-à-bouche imaginaire.`
        : `${author} réanime ${target} avec un bouche-à-bouche sensuel et profond ! || ${author} souffle un vent de passion pour réanimer ${target} au corps à corps... || Un souffle chaud de ${author} ramène ${target} à la vie avec intensité.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'reanimer');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'reanimer');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('reanimer');
      } catch (e) {
