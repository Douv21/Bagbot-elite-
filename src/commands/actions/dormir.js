...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'dormir');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('dormir', 'Dormir avec quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
