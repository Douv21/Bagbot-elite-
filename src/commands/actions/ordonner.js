...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const aiPhrase = await generateAiActionPhraseFast('ordonner', 'Ordonner à quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    const gifUrl = getValidActionGifUrl(guildId, 'ordonner');

    const randomGif = getValidActionGifUrl(guildId, 'ordonner');
    if (randomGif) {
