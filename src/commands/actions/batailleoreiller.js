...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription("Lancer une bataille d\'oreillers")
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'batailleoreiller');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('batailleoreiller', 'Lancer une bataille d\'oreillers', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} se prend un coup d\'oreiller tout seul.`
        : `${author} commence une bataille d\'oreillers intime avec ${target} au milieu du lit ! || Les oreillers volent entre ${author} et ${target}, se terminant dans un éclat de rire et d\'étreintes complices... || ${author} taquine ${target} avec un oreiller avant de s\'allonger à ses côtés.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'batailleoreiller');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'batailleoreiller');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('batailleoreiller');
      } catch (e) {
