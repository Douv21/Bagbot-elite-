...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('sodo')
    .setDescription("Faire une sodo à quelqu\'un")
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'sodo');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('sodo', 'Faire une sodo à quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} tente des acrobaties impossibles...`
        : `${author} pénètre chaudement et sodo sauvagement ${target} par derrière ! || ${author} prend le contrôle de ${target} pour une sodo brûlante et haletante... || Dans une étreinte passionnée, ${author} initie une sodo torride avec ${target}.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'sodo');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'sodo');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('sodo');
      } catch (e) {
