...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('danser')
    .setDescription("Danser avec quelqu\'un")
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'danser');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('danser', 'Danser avec quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} se lance dans une danse effrénée en solo !`
        : `${author} danse corps à corps de manière collé-serré avec ${target} ! || Dans une danse envoûtante, ${author} ondule ses hanches tout contre ${target}... || ${author} entraîne ${target} dans une danse sensuelle et rythmée.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'danser');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'danser');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('danser');
      } catch (e) {
