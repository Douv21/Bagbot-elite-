...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'vin');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('vin', 'Partager un verre de vin avec quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} boit un verre de vin en solo. Santé !`
        : `${author} partage un verre de vin enivrant dans une ambiance intime avec ${target} 🍷. || ${author} trinque et échange des regards complices avec ${target} autour d\'un bon vin... || ${author} et ${target} se rapprochent en dégustant un verre de vin en tête-à-tête.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'vin');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'vin');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('vin');
      } catch (e) {
