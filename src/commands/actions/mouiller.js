  ...
  async execute(interaction) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} s\'éclabousse tout seul.`
        : `${author} excite, caresse et mouille intensément ${target} ! || ${author} fait monter la température et rend ${target} complètement mouillé(e) de désir... || Les caresses de ${author} laissent ${target} chaud(e) et trempé(e) d\'excitation.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'mouiller');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    if (guildId) {
      ...
      try {
        gifs = getActionGifsAnyGuild('mouiller');
      } catch (e) {
