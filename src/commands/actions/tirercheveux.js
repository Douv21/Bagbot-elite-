...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('tirercheveux')
    .setDescription("Tirer les cheveux de quelqu\'un")
  ...
  async execute(interaction) {
    ...
    if (guildId) {
      const actReward = getActionReward(guildId, 'tirercheveux');
      if (actReward) {
    ...
    if (target.id !== userId) {
      ...
      const aiPhrase = await generateAiActionPhrase('tirercheveux', 'Tirer les cheveux de quelqu\'un', interaction.member, targetMember);
      if (aiPhrase) {
    ...
    if (!actionMessage) {
      actionMessage = target.id === userId 
        ? `${author} se tire les cheveux de frustration.`
        : `${author} tire fermement les cheveux de ${target} pour incliner sa tête ! || ${author} tire doucement et sensuellement les cheveux de ${target} pour intensifier leur étreinte... || Dans un moment de passion sauvage, ${author} tire les cheveux de ${target}.`;

      if (guildId) {
        const { getCustomActionMessage } = require('../../database/db');
        const customMsg = getCustomActionMessage(guildId, 'tirercheveux');
        if (customMsg) {
          actionMessage = target.id === userId
    ...
    let gifs = [];
    if (guildId) {
      gifs = getActionGifs(guildId, 'tirercheveux');
    } else {
      try {
        gifs = db.prepare('SELECT * FROM action_gifs WHERE action_name = ?').all('tirercheveux');
      } catch (e) {
