...
// ==========================================
// 🐓 COMBAT DE COQ (ARÈNE HYPER-RÉALISTE)
// ==========================================
async function handleCockfight(interaction, guildId, userId, bet, config) {
  ...
  async function startRealCockfightArena(iCtx, breedName, gearName, equippedItemExactName, playerMaxHp, critBonus, dodgeBonus, dmgMult, dmgReduction, currentBet) {
    ...
    battleCollector.on('collect', async bCtx => {
      if (bCtx.user.id !== userId) {
        return bCtx.reply({ content: '❌ Seul le dresseur d\'origine peut donner des ordres.', ephemeral: true });
      }
      ...
      if (enemyHp <= 0) {
        ...
        const victoryEmbed = new EmbedBuilder()
          ...
          .addFields(
            { name: '💰 Mise', value: `${currentBet} pièces`, inline: true },
            { name: '🎉 Gains Totaux', value: `**+${winnings} pièces** (x${mult})`, inline: true }
          )
          .setTimestamp();
